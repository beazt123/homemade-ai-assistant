...
class Engine:	
	...
	def announce(self, announcement, **kwargs):
		options = {
            'printStatements' : None,
            'indent' : ''
			}

		options.update(kwargs)

		if options["printStatements"] == None:
			printStatements = announcement
		elif isinstance(options["printStatements"], str):
			logging.debug(f"options: {options}")
			printStatements = self.formatForPPrint(options["printStatements"], 
